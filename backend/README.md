...
실제 국회 OpenAPI에서 최신 10개의 법안을 조회하여 DB에 넣고, 로컬 `gemma4:e4b` 모델을 통해 3줄 요약, 예상 영향 및 시민 호감도 점수(Sentiment)를 추출해 DB에 캐싱합니다.

> [!IMPORTANT]
> 이 명령을 실행하기 전에 **Ollama 데스크톱 앱이 실행 중**이어야 하며, `gemma4:e4b` 모델이 로컬에 받아져 있어야 합니다.
